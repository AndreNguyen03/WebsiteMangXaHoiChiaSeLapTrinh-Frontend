...
const RoleManageMainBar = () => {
  ...
  const columns = [
    { headerName: "Role ID", key: "id" },
    { headerName: "Role Name", key: "roleName" },
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5114/api/Roles") // Gọi API
      .then((response) => {
        console.log(response.data);

        // Chuyển đổi dữ liệu API theo định dạng custom
        const mappedData = response.data.map((role) => ({
          id: role.id, // Đổi id thành customId
          roleName: role.roleName, // Đổi title thành customTitle
          description: role.description, // Đổi createdAt thành customDate
          createdAt: role.createdAt, // Đổi createdAt thành customDate
          updatedAt: role.updatedAt, // Đổi createdAt thành customDate
        }));

        setRoles(mappedData); // Lưu vào state custom
      })
      .catch((error) => {
          ...
          <Table.Body className="divide-y">
            {roles.map((role) => (
              ...
              >
                ...
                <Table.Cell>{formatDate(role.updatedAt)}</Table.Cell>
                {showActions && (
